...
def main():
	...
	## STA:
	try: 
		STA = np.load('Data/RebeccaSTA.npz')
		print 'Successfully loaded {0} data STA file'.format(Data['name'])
	except IOError:
		STA = []
		print 'No {0} STA file, now generating new analysis STA files'.format(Data['name'])
	...
	try:
		...
		Cov = []
		print 'No {0} Cov file, now generating new cov analysis files'.format(Data['name'])
	...
	## Projection:
	try: 
		Project = np.load('Data/RebeccaProject.npz')
		print 'Successfully loaded {0} data Projection file'.format(Data['name'])
	except IOError:
		Project = []
		print 'No {0} data Projection file, now generating new projection analysis files'.format(Data['name'])
	if Project == []:
		Project = Neuron.FindProjection(Data,STA,Cov,180)

	## Pspike histograms to set up LN model:
	try:
		Hist = np.load('Data/RebeccaPspikeData.npz')
		print 'Successfully loaded {0} Pspike data file'.format(Data['name'])
	except IOError:
		Hist = []
		print 'No {0} Pspike file, now generating new Pspike analysis files'.format(Data['name'])
	if Hist == []:
		Hist = Neuron.PspikeHist(Project, Pspike)
	
	## LNmodel final set up
	try:
		LN = np.load('Data/RebeccaLNmodel.npz')
		print 'Successfully loaded {0} Pspike data file'.format(Data['name'])
	except IOError:
		LN = []
		print 'No {0} LN file, now generating new Pspike analysis files'.format(Data['name'])
	if LN == []
	...
	## STA:
	try :
		ModelSTA = np.load('Data/QuadSTA.npz')
	...
	## Covariance Analysis:
	try:
		ModelCov = np.load('Data/QuadCovariance.npz')
	...
	## Projection:
	try: 
		ModelProject = np.load('Data/QuadProject.npz')
	...
	## Pspike histograms to set up LN model:
	try:
		ModelHist = np.load('Data/ModelPspikeData.npz')
		print 'Successfully loaded {0} Pspike data file'.format(Data['name'])
	except IOError:
		ModelHist = []
	...
	if ModelHist == []:
		...
		## LNmodel final set up
	try:
		ModelLN = np.load('Data/RebeccaLNmodel.npz')
